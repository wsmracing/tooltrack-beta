...
export async function checkRateLimit(key: string, maximum: number, windowMs: number): Promise<RateLimitResult> {
  ...
  if (!admin) {
    console.error("Rate limit unavailable: Supabase admin client is not configured.");
  ...
  if (error || !data) {
    console.error("Rate limit unavailable: tooltrack_check_rate_limit failed.", error?.message ?? "No data returned");
    return { allowed: false, remaining: 0, resetAt: fallbackResetAt };
  }
