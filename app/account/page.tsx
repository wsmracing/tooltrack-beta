...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (profileResponse.data) {
        const value = profileResponse.data as Profile;
        setProfile(value);
        setDisplayName(value.display_name ?? "");
        setBusinessName(value.business_name ?? "");
        setPhone(value.phone ?? "");
        setSelectedPlan(value.plan_tier ?? "starter");
        setEmailNotifications(value.email_sighting_notifications !== false);
        setTeamNotifications(value.email_team_notifications !== false);
      }
  ...
  const current = getPlan(profile?.plan_tier ?? selectedPlan);
  ...
  return <div className="pageWidth pagePad accountSimple">
    ...
    <form className="accountSections" onSubmit={save}>
      ...
      <section className="settingsCard formStack"><div><h2>Account type</h2><p className="muted">Your current plan is managed by ToolTrack admin or an approved payment flow.</p></div><div className="accountTypeSummary lockedPlanSummary"><strong>{current.name}</strong><span>{current.description}</span><small>Plan changes are not available from the account page during beta.</small></div></section>
