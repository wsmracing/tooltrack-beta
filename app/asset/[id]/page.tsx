...
export default function AssetPage() {
  ...
  const [galleryPhoto, setGalleryPhoto] = useState<StoredFile | null>(null);
  ...
  async function assetAction(body: Record<string, unknown>) {
    ...
    setMessage(result.message || "Asset updated.");
    await load();
  }

  async function report(event: FormEvent) {
    event.preventDefault(); if (!asset || !confirmed) return;
    if (!window.confirm(`Report ${asset.make} ${asset.model} (${asset.serial_original}) as stolen?`)) return;
    setSaving(true); setError(""); setMessage("");
    try { await assetAction({ action: "report_stolen", theftDate: date, locationArea: location, gardaReference: gardaRef, circumstances }); setReportOpen(false); setConfirmed(false); }
    catch (caught) { setError(friendlyError(caught, "The stolen report could not be saved.")); }
  ...
  async function recover() {
    ...
    setSaving(true); setError(""); setMessage("");
    try { await assetAction({ action: "recover" }); }
    catch (caught) { setError(friendlyError(caught, "The asset could not be marked recovered.")); }
    finally { setSaving(false); }
  }

  async function markForSale() {
    setSaving(true); setError(""); setMessage("");
    try { await assetAction({ action: "mark_for_sale", days: 14 }); setSaleOpen(false); }
    catch (caught) { setError(friendlyError(caught, "Sale status could not be activated.")); }
    finally { setSaving(false); }
  }

  async function removeFromSale() {
    setSaving(true); setError(""); setMessage("");
    try { await assetAction({ action: "remove_from_sale" }); setSaleOpen(false); }
    catch (caught) { setError(friendlyError(caught, "Sale status could not be removed.")); }
    finally { setSaving(false); }
  }
  ...
  async function createTransfer(event: FormEvent) {
    ...
    } catch (caught) { setError(friendlyError(caught, "The transfer could not be created.")); }
    finally { setSaving(false); }
  }
  ...
  async function cancelTransfer(transferId: string) {
    ...
    if (transferError) setError(friendlyError(transferError, "The transfer could not be cancelled.")); else { setMessage("Transfer cancelled."); await load(); }
  }

  async function confirmBuyer(event: FormEvent) {
    event.preventDefault();
    if (buyerCode.replace(/\D/g, "").length !== 6) return;
    setSaving(true); setError("");
    try {
      const token = await accessToken();
      const response = await fetch("/api/seller-confirmations", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ assetId: id, code: buyerCode }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "The buyer check could not be confirmed.");
      setMessage(body.message); setBuyerCode(""); setConfirmOpen(false); await load();
    } catch (caught) { setError(friendlyError(caught, "The buyer check could not be confirmed.")); }
    finally { setSaving(false); }
  }
  ...
  async function gardaPdf() {
    ...
    try {
      const supabase = getSupabaseBrowser();
      const photoDataUrls = await Promise.all(photos.slice(0, 6).map(async (photo) => {
        const { data, error: downloadError } = await supabase.storage.from("asset-photos").download(photo.storage_path);
      ...
      await downloadGardaEvidenceReport({ asset, owner: { email: user.email, profile }, theftReport, audit, photoCount: photos.length, documentCount: documents.length, photoDataUrls: photoDataUrls.filter((value): value is string => Boolean(value)) });
    } catch (caught) { setError(friendlyError(caught, "The theft evidence PDF could not be created.")); }
  }

  async function insurancePdf() {
    if (!asset || !user) return;
    try { await downloadInsuranceSchedule([asset], { email: user.email, profile }); }
    catch (caught) { setError(friendlyError(caught, "The insurance PDF could not be created.")); }
  }
  ...
  return <div className="pageWidth pagePad assetDetailPage">
    ...
    {galleryPhoto && <div className="modalBackdrop galleryBackdrop" onClick={() => setGalleryPhoto(null)}><div className="galleryModal" onClick={(event) => event.stopPropagation()}><div className="galleryHeader"><div><strong>{galleryPhoto.original_name}</strong><span>{new Date(galleryPhoto.created_at).toLocaleDateString("en-IE")}</span></div><button type="button" className="button secondary" onClick={() => setGalleryPhoto(null)}>Close</button></div><div className="galleryImageFrame">{photoUrls[galleryPhoto.storage_path] ? <img src={photoUrls[galleryPhoto.storage_path]} alt={galleryPhoto.original_name} /> : <FileIcon />}</div></div></div>}
