...
export async function POST(request: NextRequest) {
  ...
  if (!rate.allowed) {
    return NextResponse.json(
      { error: "Too many confirmation requests. Wait a minute and try again." },
      { status: 429, headers: { "Retry-After": retryAfterSeconds(rate.resetAt), "Cache-Control": "no-store" } },
    );
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (!rate.allowed) {
    return NextResponse.json(
      { error: "Too many confirmation attempts. Wait and try again." },
      { status: 429, headers: { "Retry-After": retryAfterSeconds(rate.resetAt), "Cache-Control": "no-store" } },
    );
  }
  ...
  if (new Date(challenge.expires_at).getTime() <= Date.now()) {
    await admin.from("seller_confirmation_challenges").update({ status: "expired" }).eq("id", challenge.id).eq("status", "pending");
    return NextResponse.json({ error: "That confirmation code is invalid or has expired." }, { status: 400 });
  }

  if (!safeCompare(hash(code), challenge.challenge_code_hash)) {
    ...
    await admin
      .from("seller_confirmation_challenges")
      .update({ failed_attempts: failedAttempts, locked_at: lockedAt, last_failed_at: new Date().toISOString() })
      .eq("id", challenge.id)
      .eq("status", "pending")
      .is("locked_at", null);
    return NextResponse.json(
      { error: lockedAt ? "This confirmation check is locked after too many wrong attempts." : "That confirmation code is invalid or has expired." },
      { status: lockedAt ? 423 : 400 },
    );
  }
