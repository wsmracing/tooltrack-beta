...
-- Compatibility with the earlier beta shop table, where SKU is mandatory.
-- Give SKU a server-side default so V4.1 seed data and Shop Admin inserts work
-- without exposing SKU as a required field in the prototype interface.
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='shop_products' and column_name='sku'
  ) then
    execute $sql$
      alter table public.shop_products
...
-- Keep legacy `active` and V4.1 `is_active` values aligned when both exist.
...
begin
  if exists (
    ...
    select 1 from information_schema.columns
    where table_schema='public' and table_name='shop_products' and column_name='is_active'
  ) then
